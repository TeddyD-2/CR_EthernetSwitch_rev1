...
static void capPortSpeed(KSZ9477 &sw, uint8_t port, uint16_t maxMbps)
{
  ...
  // PHY reg 9 (1000BASE-T Control) @ 0x112 — adv 1000-FD/HD = bits 9,8
  ...
  // PHY reg 4 (ANAR) @ 0x108 — adv 100-FD/HD = bits 8,7; 10-FD/HD = 6,5
  uint16_t anar = sw.readReg16(base | 0x0108);
  if (maxMbps >= 100) anar |=  ((1u << 8) | (1u << 7));
  else                anar &= ~((1u << 8) | (1u << 7));
  anar |= (1u << 6) | (1u << 5);   // always advertise 10-FD/HD
  sw.writeReg16(base | 0x0108, anar);

  // Restart auto-negotiation: BMCR (reg 0) @ 0x100, bit 9
